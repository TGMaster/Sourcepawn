...
public void PugSetup_OnReadyToStartCheck(int readyPlayers, int totalPlayers) {
  ...
  if (need >= 1) {
    Format(hostname, sizeof(hostname), "%s [%i / %i]", g_HostName, totalPlayers, PugSetup_GetPugMaxPlayers());
  } else {
    Format(hostname, sizeof(hostname), "%s [WARM UP]", g_HostName);
  }
...
public Action Event_RoundStart(Event event, const char[] name, bool dontBroadcast) {
  ...
  char hostname[MAX_HOST_LENGTH];
  Format(hostname, sizeof(hostname), "%s [%i / %i]", g_HostName, CS_GetTeamScore(CS_TEAM_CT),
