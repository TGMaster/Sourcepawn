...
PrintRemainingHealth()
{
        ...
        CPrintToChatAll("Tank ({olive}%s{default}) had {red}%d{default} health remaining", name, g_iLastTankHealth);
}
     
PrintTankDamage()
{
        if (!g_bEnabled) return;
        PrintToChatAll("Damage Tank Report:");
        ...
        for (new i; i <= survivor_index; i++)
        {
                ...
                for (int j = 1; j <= MaxClients; j++)
                	if (IsClientInGame(j) && !IsFakeClient(j))
                		CPrintToChat(j, "{blue}%4d{default}\t[{olive}%d%%{default}]:\t{blue}%N", damage, percent_damage, client);
        }
}
