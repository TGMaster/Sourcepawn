...
public OnPluginStart() {
	...
	// Wave interval
	hCvarWaveType = CreateConVar("siws_wave_type", "1");
	hCvarWaveInterval = CreateConVar("siws_wave_interval", "25", "Interval in seconds between special infected waves");
	
	// Custom class limits
	hCvarMaxSpecials 	= CreateConVar("siws_max_specials", 	"3", "Maximum Specials alive at any time");
	...
	hCvarHunterLimit 	= CreateConVar("siws_hunter_limit", 	"0", "Maximum hunters alive at any time");
...
PrintSettings() {
	new String:buf[1024];
	new String:buf2[1024];
	new String:tmp[128];
	Format(tmp, sizeof(tmp), "%d", GetConVarInt(hCvarMaxSpecials));
	StrCat(buf, sizeof(buf), tmp);
	StrCat(buf2, sizeof(buf2), "Max Specials");
	if (GetConVarInt(hCvarHunterLimit) > 0) {
		Format(tmp, sizeof(tmp), "                        %d", GetConVarInt(hCvarHunterLimit));
		StrCat(buf, sizeof(buf), tmp);
		StrCat(buf2, sizeof(buf2), " | Max Hunters");
	}
	if (GetConVarInt(hCvarSmokerLimit) > 0){
		Format(tmp, sizeof(tmp), "                        %d", GetConVarInt(hCvarSmokerLimit));
		StrCat(buf, sizeof(buf), tmp);
		StrCat(buf2, sizeof(buf2), " | Max Smokers");
	}
	if (GetConVarInt(hCvarJockeyLimit) > 0){
		Format(tmp, sizeof(tmp), "                        %d", GetConVarInt(hCvarJockeyLimit));
		StrCat(buf, sizeof(buf), tmp);
		StrCat(buf2, sizeof(buf2), " | Max Jockeys");
	}
	if (GetConVarInt(hCvarChargerLimit) > 0){
		Format(tmp, sizeof(tmp), "                        %d", GetConVarInt(hCvarChargerLimit));
		StrCat(buf, sizeof(buf), tmp);
		StrCat(buf2, sizeof(buf2), " | Max Chargers");
	}
	if (GetConVarInt(hCvarBoomerLimit) > 0){
		Format(tmp, sizeof(tmp), "                        %d", GetConVarInt(hCvarBoomerLimit));
		StrCat(buf, sizeof(buf), tmp);
		StrCat(buf2, sizeof(buf2), " | Max Boomers");
	}
	if (GetConVarInt(hCvarSpitterLimit) > 0){
		Format(tmp, sizeof(tmp), "                        %d", GetConVarInt(hCvarSpitterLimit));
		StrCat(buf, sizeof(buf), tmp);
		StrCat(buf2, sizeof(buf2), " | Max Spitters");
	}
	PrintToChatAll(buf2);
	PrintToChatAll(buf);
		...
		if (g_bHasPassedBaitThreshold) {
			...
			if (g_bIsTankInPlay) {
				...
				if (g_bIsSpawnerActive) {
					...
					g_iCount = 0;
					g_bIsSpawnerActive = false;
					if (GetConVarInt(hCvarWaveType) == 2)
						CreateTimer(GetConVarFloat(hCvarWaveInterval), Timer_ActivateSpawner, _, TIMER_FLAG_NO_MAPCHANGE);
				}
				if (GetConVarInt(hCvarWaveType) == 1) {
					new max = GetConVarInt(hCvarMaxSpecials);
					if (g_iCount >= max)
						CreateTimer(GetConVarFloat(hCvarWaveInterval), Timer_ActivateSpawner, _, TIMER_FLAG_NO_MAPCHANGE);
				}
			}
...
public SetLimitCoop() {
	SetConVarBool(FindConVar("z_hunter_limit"), true);
	SetConVarBool(FindConVar("z_charger_limit"), true);
	SetConVarBool(FindConVar("z_smoker_limit"), true);
	SetConVarBool(FindConVar("z_spitter_limit"), true);
	SetConVarBool(FindConVar("z_boomer_limit"), true);
	SetConVarBool(FindConVar("z_jockey_limit"), true);
}