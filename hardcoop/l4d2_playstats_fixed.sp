...
String: GetMVPChatString( bool:bRound = true, bool:bTeam = true, iTeam = -1 )
{
    ...
    if ( !(iBrevityFlags & BREV_FF) )
    {
        if ( mvp_FF == -1 )
        {
            Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}LVP%s{default}]  FF: (no friendly fire at all!)\n",
                    (bRound) ? "" : " - Game"
                );
            StrCat(printBuffer, sizeof(printBuffer), tmpBuffer);
        }
        ...
        {
            Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}LVP%s{default}]  FF:{olive} %s {default}({blue}%d {default}dmg)\n",
                        (bRound) ? "" : " - Game",
