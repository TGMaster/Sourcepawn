...
public OnPluginStart() {
	RegConsoleCmd("sm_song", Command_MusicMenu, "Listen to music");
	RegConsoleCmd("sm_amnhac", Command_MusicMenu, "Listen to music");
	...
	djUrlCvar = CreateConVar("smdj_url", "http://blazersvn.ddns.net/smdj", "The URL to your SMDJ Web installation. The proper value of this variable is displayed in the admin panel of your Web installation.");
	djUrlPortCvar = CreateConVar("smdj_url_port", "80", "If your web installation is hosted on a different port, set it here");
	authKeyCvar = CreateConVar("smdj_auth_token", "7c33700644d6e78504cde5ebc464e7de", "The auth token displayed in the admin panel of your SMDJ Web installation", FCVAR_PROTECTED);
...
public Action:Command_SongList(client, args) {
	if(client == 0) {
		ReplyToCommand(client, "[Music] This command can only be used from in-game.");
...
public Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	PrintToChat(client, "\x03[Music] \x01Type \x04!dj \x01or \x04!music \x01in chat to listen to music.");
	advertShown[client] = true;
}

public Action:Command_RandomSong(client, args) {
	if(songMenu == INVALID_HANDLE) {
		PrintToChat(client, "\x03[Music] \x01There are no songs to play.");
		return Plugin_Handled;
	}
...
public Action:Command_MusicMenu(client, args) {
	...
	if(!AreClientCookiesCached(client)) {
		ReplyToCommand(client, "\x04[Music] \x01List is not ready yet.");
		return Plugin_Handled;
	}
	if(playlistArray[client] != INVALID_HANDLE) {
	...
	if(args != 0) {
		...
		if(index == -1) {
			CPrintToChat(client, "{olive}[Music] {default}The requested song title was not found.");
			return Plugin_Handled;
		} else {
	...
	if(songMenu == INVALID_HANDLE) {
		CPrintToChat(client, "{olive}[Music] {default}There are no songs to display.");
		return Plugin_Handled;
	}
	...
	SetMenuTitle(menu, "Blazers Music");
...
public Handler_SelectTarget(Handle:menu, MenuAction:action, client, param) {
	...
	if(songToPlay[client] != -1) {
		...
		ShowActivity2(client, "\x04[Music] \x05", "\x01Played song \x05%s \x01to \x05%s", title, target_name);
	} else {
		ShowActivity2(client, "\x04[Music] \x05", "\x01Stopped playing music on \x05%s", target_name);
	}
	for(new i = 0; i < total; i++) {
		if(songToPlay[target_list[i]] == -1) {
			...
			PrintToChat(target_list[i], "\x04[Music] \x01You are now listening to \x05%s", title);
...
public Handler_NewPlaylist(Handle:menu, MenuAction:action, client, param) {
	...
	if(StrEqual(selection, "done")) {
		...
		CloseHandle(playlistArray[client]);
		playlistArray[client] = INVALID_HANDLE;
		PrintToChat(client, "\x04[Music] \x01You have cancelled creating your playlist.");
		return;
	}
...
public OnPostReceive(Handle:socket, const String:receiveData[], dataSize, any:pack) {
	if(StrContains(receiveData, "Bad auth token", false) != -1 || StrContains(receiveData, "Invalid data", false) != -1) {
		...
		if(method == 1) {
			CPrintToChat(client, "{olive}[Music] {default}An error occurred when adding your playlist. Contact the server administrator.");
		} else if(method == 2) {
			CPrintToChat(client, "{olive}[Music] {default}An error occurred when deleting your playlist. Contact the server administrator.");
		} else {
			CPrintToChat(client, "{olive}[Music] {default}An error occurred when modifying your playlist. Contact the server administrator.");
		}
	}
...
public OnPostDisconnected(Handle:socket, any:pack) {
	...
	new method = ReadPackCell(pack);
	if(method == 1) {
		CPrintToChat(client, "{olive}[Music] {default}Your playlist has been added.");
	} else if(method == 2) {
		CPrintToChat(client, "{olive}[Music] {default}Your playlist has been deleted.");
	} else {
		CPrintToChat(client, "{olive}[Music] {default}Your playlist has been modified.");
	}
	LoadSongs();
	CloseHandle(socket);
	CloseHandle(pack);
...
PlaySong(client, index, bool:playlist = false, bool:silent = false) {
	...
	if(!warningShown[client] && GetConVarBool(helpAdvertCvar)) {
		PrintHintText(client, "If you cannot hear the music, please wait or choose other songs.");
		//CPrintToChat(client, "{olive}[Music] {default}You must have Adobe Flash Player for Other Browsers installed to listen to music. Type {green}!musichelp {default}for help.");
		warningShown[client] = true;
	}
	CPrintToChat(client, "{olive}[Music] {default}Type {green}!musicoff {default}to stop the music or {green}!musicinfo {default}for controls.");
	if(index == -2) {
		if(GetConVarBool(advertCvar) && !silent) {
			CPrintToChatAllEx(client, "{olive}[Music] {teamcolor}%N {default}is listening to {green}all songs (shuffled)", client);
		}
		OpenURL(client, -2);
		return;
	}
	if(playlist) {
		if(GetConVarBool(advertCvar) && !silent) {
			CPrintToChatAllEx(client, "{olive}[Music] {teamcolor}%N {default}is listening to {green}a personal playlist", client);
		}
	...
	if(GetConVarBool(advertCvar) && !silent) {
		CPrintToChatAllEx(client, "{olive}[Music] {teamcolor}%N {default}is listening to {green}%s", client, title);
	}
...
public Action:Command_Debug_DumpArrays(client, args) {
	...
	CloseHandle(file);
	ReplyToCommand(client, "[Music] The arrays have been dumped to data/smdj_arrays_dump.txt");
	return Plugin_Handled;
}
