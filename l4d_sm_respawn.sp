...
{
	name = "L4D SM Respawn",
	author = "AtomicStryker & Ivailosp & Blazers Team",
...
public Action:Command_Revive(client, args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "[SM] Usage: sm_respawn2 <player1> [player2] ... [playerN] - respawn all listed players");
	...
	{
		RevivePlayer(client, target_list[i]);
	}
	
	ShowActivity2(client, "[SM] ", "Revived target '%s'", target_name);
