...
CVote_DoVote(client, const String:votename[], const String:vparams[][], vparamct, bool:fromtrigger=false) {
	...
	if (cvote[type] == CVoteType_Chat) {
		...
		switch(cvote[type]) {
			...
			case CVoteType_Confirm: {
				...
				if (!IsBuiltinVoteInProgress())//disregard sm_vote_delay
				{
					...
					Format(sBuffer, sizeof(sBuffer), votetitle);
				...
				else PrintToChat(client, "Vote cannot be started now.");
			}
		...
		VoteMenu(vm, iPlayers, iNumPlayers, 20);
	}
}
		...
		if (item_info[i][BUILTINVOTEINFO_ITEM_INDEX] == BUILTINVOTES_VOTE_YES)
		{
			if (item_info[i][BUILTINVOTEINFO_ITEM_VOTES] > (num_clients / 2))
			{
				if (vote == g_hMatchVote)
				{
					...
					new Handle:strpack = CreateDataPack();
					WritePackString(strpack, execcommand);
					DisplayBuiltinVotePass(vote, "Vote has been accepted...");
					CreateTimer(GetConVarFloat(sm_cvote_executedelay), Timer_ExecuteCommand, strpack, TIMER_FLAG_NO_MAPCHANGE);
					return;
				}
			}
		}
