...
public Action:ListRates(client, args) 
{
	...
	CPrintToChat(client, "{blue}[{default}RATE{blue}]{default} List of player netsettings (cmd/upd/rate):");
	...
	for (new i = 0; i < GetArraySize(hClientSettingsArray); i++) 
	{
		...
		{
			CPrintToChat(client, "{olive}%N{default} :\t{blue}%d/%d/%d", iClient, player[Client_Cmdrate], player[Client_Updaterate], player[Client_Rate]);
		}
	}
...
{	
    ...
    if (iIndex > -1) 
    {
        ...
        CPrintToChat(client, "{blue}[{default}RATE{blue}]{default} Your netset changed from {olive}%d{default}/{olive}%d{default}/{green}%d{default} to {olive}%d{default}/{olive}%d{default}/{green}%d{default}%s", 
    ...
    else
    {
        ...
        //CPrintToChat(client, "Your netsettings set to \x04%d\x01/\x04%d\x01/\x05%d\x01", player[Client_Cmdrate], player[Client_Updaterate], player[Client_Rate]);
    }
}
