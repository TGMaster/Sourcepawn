...
stock DisplayStatsMVPChat( client, bool:bRound = true, bool:bTeam = true, iTeam = -1 )
{
        ...
        for ( i = 1; i < g_iTeamSize && i < g_iPlayers; i++ )
        {
            ...
            if ( listNumber && ( client == -1 || client == found ) && IS_VALID_CLIENT(found) && !IsFakeClient(found) && g_iCookieValue[found] != -1 )
            {
                if ( iBrevityFlags & BREV_PERCENT ) {
                	Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}MVP%s{default}] SI rank {green}#%d {default}({blue}%d {default}dmg,{blue} %d {default}kills)",
                            (bRound) ? "" : " - Game",
                            (i+1),
                	...
                	Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}MVP%s{default}] SI rank {green}#%d {default}(dmg{green} %i%%{default}, kills{green} %i%%{default})",
                            (bRound) ? "" : " - Game",
                            (i+1),
                            RoundFloat( (bRound) ?
                	...
                	Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}MVP%s{default}] SI rank {green}#%d {default}({blue}%d {default}dmg [{green}%i%%{default}], {blue}%d {default}kills [{green}%i%%{default}])",
                            (bRound) ? "" : " - Game",
                            (i+1),
                            (bRound) ? g_strRoundPlayerData[index][team][plySIDamage] : g_strPlayerData[index][plySIDamage],
                            RoundFloat( (bRound) ? 
                ...
                CPrintToChat( found, "%s", tmpBuffer );
            }
    ...
    if (    ( bRound && g_strRoundData[g_iRound][team][rndCommon] || !bRound && g_strAllRoundData[team][rndCommon] )
        ...
        for ( i = 1; i < g_iTeamSize && i < g_iPlayers; i++ )
        {
            ...
            {
                if ( iBrevityFlags & BREV_PERCENT ) {
                	Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}MVP%s{default}] CI rank {green}#%d {default}({blue}%d {default}kills)",
                            (bRound) ? "" : " - Game",
                            (i+1),
                            (bRound) ? g_strRoundPlayerData[index][team][plyCommon] : g_strPlayerData[index][plyCommon]
                        );
                } else if (iBrevityFlags & BREV_ABSOLUTE) {
                	Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}MVP%s{default}] CI rank {green}#%d {default}(kills{green} %i%%{default})",
                            (bRound) ? "" : " - Game",
                            (i+1),
                            RoundFloat( (bRound) ?
                	...
                	Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}MVP%s{default}] CI rank {green}#%d {default}({blue}%d {default}kills [{green}%i%%{default}])",
                            (bRound) ? "" : " - Game",
                            (i+1),
                            (bRound) ? g_strRoundPlayerData[index][team][plyCommon] : g_strPlayerData[index][plyCommon],
                            RoundFloat( (bRound) ? 
    ...
    if (    !(iBrevityFlags & BREV_RANK) && !(iBrevityFlags & BREV_FF) )
    {
        ...
        for ( i = 1; i < g_iTeamSize && i < g_iPlayers; i++ )
        {
            ...
            if ( listNumber && ( client == -1 || client == found ) && IS_VALID_CLIENT(found) && !IsFakeClient(found) && g_iCookieValue[found] != -1 )
            {
            	Format(tmpBuffer, sizeof(tmpBuffer), "[{red}LVP%s{default}]  FF rank {green}#%d {default}({red}%d {default}dmg)",
                        (bRound) ? "" : " - Game",
                        (i+1),
...
String: GetMVPChatString( bool:bRound = true, bool:bTeam = true, iTeam = -1 )
{
    ...
    {
    	Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}MVP%s{default}]: (not enough action yet)\n", (bRound) ? "" : " - Game" );
    ...
    else
    {
        if ( !(iBrevityFlags & BREV_SI) )
        {
            ...
            {
                if ( iBrevityFlags & BREV_PERCENT ) {
                	Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}MVP%s{default}] SI: {olive}%s {default}({blue}%d {default}dmg,{blue} %d {default}kills)\n", 
                            (bRound) ? "" : " - Game",
                    ...
                    Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}MVP%s{default}] SI:{olive} %s {default}(dmg{green} %i%%{default}, kills{green} %i%%{default})\n",
                            (bRound) ? "" : " - Game",
                            g_sPlayerName[mvp_SI],
                            RoundFloat( (bRound) ?
                	...
                	Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}MVP%s{default}] SI:{olive} %s {default}({blue}%d {default}dmg [{green}%i%%{default}],{blue} %d {default}kills [{green}%i%%{default}])\n",
                            (bRound) ? "" : " - Game",
                            g_sPlayerName[mvp_SI],
                            (bRound) ? g_strRoundPlayerData[mvp_SI][team][plySIDamage] : g_strPlayerData[mvp_SI][plySIDamage],
                            RoundFloat( (bRound) ?
            ...
            {
            	Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}MVP%s{default}] SI: {olive}(nobody){default}\n", (bRound) ? "" : " - Game" );
                StrCat(printBuffer, sizeof(printBuffer), tmpBuffer);
            }
        }
        ...
        if ( !(iBrevityFlags & BREV_CI) )
        {
            ...
            if (    mvp_Common > -1
                ...
                if ( iBrevityFlags & BREV_PERCENT ) {
                	Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}MVP%s{default}] CI:{olive} %s {default}({blue}%d {default}common)\n",
                            (bRound) ? "" : " - Game",
                            g_sPlayerName[mvp_Common],
                            (bRound) ? g_strRoundPlayerData[mvp_Common][team][plyCommon] : g_strPlayerData[mvp_Common][plyCommon]
                        );
                } else if ( iBrevityFlags & BREV_ABSOLUTE ) {
                	Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}MVP%s{default}] CI:{olive} %s {default}({green}%i%%{default})\n",
                            (bRound) ? "" : " - Game",
                            g_sPlayerName[mvp_Common],
                            RoundFloat( (bRound) ? 
                	...
                	Format(tmpBuffer, sizeof(tmpBuffer), "[{blue}MVP%s{default}] CI:{olive} %s {default}({blue}%d {default}common [{green}%i%%{default}])\n",
                            (bRound) ? "" : " - Game",
                            g_sPlayerName[mvp_Common],
                            (bRound) ? g_strRoundPlayerData[mvp_Common][team][plyCommon] : g_strPlayerData[mvp_Common][plyCommon],
                            RoundFloat( (bRound) ? 
    ...
    if ( !(iBrevityFlags & BREV_FF) )
    {
        if ( mvp_FF == -1 )
        {
            Format(tmpBuffer, sizeof(tmpBuffer), "[{red}LVP%s{default}]  FF: (no friendly fire at all!)\n",
                    (bRound) ? "" : " - Game"
                );
            StrCat(printBuffer, sizeof(printBuffer), tmpBuffer);
        }
        ...
        {
        	Format(tmpBuffer, sizeof(tmpBuffer), "[{red}LVP%s{default}]  FF:{olive} %s {default}({red}%d {default}dmg)\n",
                        (bRound) ? "" : " - Game",
...
stock DisplayStatsFunFactChat( client, bool:bRound = true, bool:bTeam = true, iTeam = -1 )
{
    ...
    if ( client > 0 ) {
        for ( i = 0; i < intPieces; i++ ) {
            CPrintToChat(client, "\x01%s", strLines[i]);
        }
    }
    else if ( client == 0 ) {
        for ( i = 0; i < intPieces; i++ ) {
        ...
        for ( j = 1; j <= MaxClients; j++ ) {
            for ( i = 0; i < intPieces; i++ ) {
                if ( !IS_VALID_INGAME( j ) || g_iCookieValue[j] != 0 ) { continue; }
                CPrintToChat( j, "\x01%s", strLines[i] );
            }
        }
    }
}
