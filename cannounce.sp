...
PrintFormattedMessageToAll( String:rawmsg[364], client )
{
	decl String:message[364];
...
PrintFormattedMessageToAdmins( String:rawmsg[364], client )
{
	decl String:message[364];
...
PrintFormattedMsgToNonAdmins( String:rawmsg[364], client )
{
	decl String:message[364];
...
GetFormattedMessage( String:rawmsg[364], client, String:outbuffer[], outbuffersize )
{
	...
	if( client > -1 )
	{
		...
		if (StrContains(rawmsg, "{LERP}") != -1 )
		{
			new Float:number = newLerpTime * 1000;
			Format(sBuffer, sizeof(sBuffer), "%.1f", number);
			ReplaceString(rawmsg, sizeof(rawmsg), "{LERP}", sBuffer);
		}
