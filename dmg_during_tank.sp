...
public player_hurt(Handle:event, const String:name[], bool:dontBroadcast) {
    ...
    if (attacker <= 0 ||
    	...
    	GetClientTeam(attacker) != 2) return;
...
PrintDamageDealtToTank(tank, sortArray[]) {
    ...
    for (new client=1; client<=MaxClients; client++) {
        if (client <= 0 || client > MaxClients || !IsClientInGame(client)) continue;
        ...
        for (new i=0; i<sizeof(damageToTank); i++) {
            ...
            PrintToChat(client, "\x05%4d\x01 [\x04%.0f%%\x01]:\t\x03%N\x01", damageToTank[j], damageToTank[j]*100.0/tankHealth, j);
        }
    }
}
