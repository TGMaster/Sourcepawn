...
public Action:HudDrawTimer(Handle:hTimer) 
{
	...
	if (bSpecsOnServer) // Only bother if someone's watching us
	{
		...
		for (new i = 1; i <= MaxClients; i++) 
		{
			...
			if (!bSpecHudHintShown[i])
			{
				...
				CPrintToChat(i, "[{olive}HUD{default}] Type {green}!spechud{default} to toggle the {blue}Spectator HUD{default}.");
			}
		}
	...
	for (new i = 1; i <= MaxClients; i++) 
	{
		...
		if (!bTankHudHintShown[i])
		{
			...
			CPrintToChat(i, "[{olive}HUD{default}] Type {green}!tankhud{default} to toggle the {red}Tank HUD{default}.");
		}
	}
...
FillSurvivorInfo(Handle:hSpecHud) 
{
	...
	for (new client = 1; client <= MaxClients && survivorCount < GetConVarInt(survivor_limit); client++) 
	{
		...
		else
		{
			...
			{
				Format(info, sizeof(info), "%s: %iHP <Hang> [%s]", name, GetSurvivorHealth(client), info);
			}
			else if (IsIncapacitated(client))
			{
				Format(info, sizeof(info), "%s: %iHP (#%i) [%s]", name, GetSurvivorHealth(client), (GetSurvivorIncapCount(client) + 1), info);
			}
			else
			{
				...
				{
					Format(buffer, sizeof(buffer), "%i", incapCount);
