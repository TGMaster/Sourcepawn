...
#define PLUGIN_NAME "Player Stats"
...
public OnPluginStart()
{
	...
	// Administrative Cvars
	cvar_AdminPlayerCleanLastOnTime = CreateConVar("l4d_stats_adm_cleanoldplayers", "1", "How many months old players (last online time) will be cleaned. 0 = Disabled", FCVAR_PLUGIN, true, 0.0);
	...
	cvar_HumansNeeded = CreateConVar("l4d_stats_minhumans", "1", "Minimum Human players before stats will be enabled", FCVAR_PLUGIN, true, 1.0, true, 4.0);
	...
	cvar_AnnounceMotd = CreateConVar("l4d_stats_announcemotd", "0", "Chat announcment for the message of the day.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
public Action:AnnounceConnect(Handle:timer, any:client)
{
	...
	ShowMOTD(client);
	StatsPrintToChat2(client, true, "Type \x04!rankmenu \x01to operate \x04%s\x01!", PLUGIN_NAME);
}
...
public Action:timer_UpdatePlayers(Handle:timer, Handle:hndl)
{
	if (CheckHumans())
	{
		...
		{
			StatsPrintToChatAllPreFormatted("Left 4 Dead Stats are \x04DISABLED\x01, not \x03enough Human \x01players!");
		}
	...
	if (GetArraySize(SurvivorArray) > 0 || GetArraySize(InfectedArray) > 0)
	{
		...
		else
		{
			...
			for (i = 0; i < GetArraySize(InfectedArray); i++)
			{
				...
				StatsPrintToChatAll2(true, "\x04%s \x01was swapped to team \x03Survivors\x01!", Name);
			}
			...
			for (i = 0; i < GetArraySize(SurvivorArray); i++)
			{
				...
				StatsPrintToChatAll2(true, "\x04%s \x01was swapped to team \x03Infected\x01!", Name);
			}
...
public RankVotePanelHandler(Handle:menu, MenuAction:action, param1, param2)
{
	...
	if (param2 == 1 || param2 == 2)
	{
		...
		StatsPrintToChatAll("\x04%s \x01voted (\x03%i/%i\x01)", Name, votes, humans);
	}
}
...
public DisplayClearPanelHandler(Handle:menu, MenuAction:action, param1, param2)
{
	...
	if (DoFastQuery(param1, "DELETE FROM %splayers WHERE steamid = '%s'", DbPrefix, SteamID))
	{
		...
		if (client != param1)
			StatsPrintToChat(param1, "Player \x04%s \x01stats cleared!", Name);
	}
	else
...
public StatsPrintToChatPreFormatted2(Client, bool:Forced, const String:Message[])
{
	...
	PrintToChat(Client, "\x01[\x03Stats\x01] \x01%s", Message);
}
