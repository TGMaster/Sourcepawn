...
#define WM_VERSION				"16.12.05.1622"
...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_nrdy", ReadyDown, "Readies down the client");
	...
	/* Veto cmds */
	RegConsoleCmd("sm_mapbanbo1", Veto_Bo1, "Ask for a Bo1 Veto");
	RegConsoleCmd("sm_mapbanbo2", Veto_Bo2, "Ask for a Bo2 Veto");
	RegConsoleCmd("sm_mapbanbo3", Veto_Bo3, "Ask for a Bo3 Veto");
	RegConsoleCmd("sm_mapbanbo5", Veto_Bo5, "Ask for a Bo5 Veto");
	RegConsoleCmd("sm_mapban", Veto_Setup, "Ask for Veto");
	RegConsoleCmd("sm_mapban1", Veto_Bo1, "Ask for a Bo1 Veto");
	RegConsoleCmd("sm_mapban2", Veto_Bo2, "Ask for a Bo2 Veto");
	RegConsoleCmd("sm_mapban3", Veto_Bo3, "Ask for a Bo3 Veto");
	RegConsoleCmd("sm_mapban5", Veto_Bo5, "Ask for a Bo5 Veto");
	RegConsoleCmd("sm_mapbanmaps", Veto_Bo3_Maps, "Veto Bo3 Maps");
	...
	RegAdminCmd("forceready", ForceAllReady, ADMFLAG_CUSTOM1, "Forces all players to become ready");
	RegAdminCmd("fr", ForceAllReady, ADMFLAG_CUSTOM1, "Forces all players to become ready");
	RegAdminCmd("forceunready", ForceAllUnready, ADMFLAG_CUSTOM1, "Forces all players to become unready");
	RegAdminCmd("fnr", ForceAllUnready, ADMFLAG_CUSTOM1, "Forces all players to become unready");
	RegAdminCmd("forcespectate", ForceAllSpectate, ADMFLAG_CUSTOM1, "Forces all players to become a spectator");
	RegAdminCmd("forcespec", ForceAllSpectate, ADMFLAG_CUSTOM1, "Forces all players to become a spectator");
	...
	wm_competition = CreateConVar("wm_competition", "IU Gaming Tour", "Name of host for a competition. eg. ESEA, Cybergamer, CEVO, ESL");
...
static void LiveText() {
	if (LiveOn2) {
		...
		PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 %t \x03IU Gaming Tour", CHAT_PREFIX, "Powered By");
		...
		if (GetConVarBool(wm_knife_zeus)) {
			...
			PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 \x0E%t", CHAT_PREFIX, "Good Luck");
			PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 %t \x03IU Gaming Tour", CHAT_PREFIX, "Powered By");
		} else {
			PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 \x02%t", CHAT_PREFIX, "Knife");
			PrintToChatAll("\x01 \x09[\x04%s\x09]\x01 \x0E%t", CHAT_PREFIX, "Good Luck");
...
void DispInfo(int client, char[] players_unready, int time)
{
	if (GetConVarBool(wm_ready_panel)) {
		...
		SetGlobalTransTarget(client);
		g_m_ready_up = CreatePanel();
		Format(Temp, sizeof(Temp), "%t", "Ready System");
...
public Action ShowPluginInfo(Handle timer, int client)
{
	if (client != 0 && IsClientConnected(client) && IsClientInGame(client))
	{
		...
		PrintToConsole(client, "  /unready - Mark yourself as not ready 	  /nrdy /nr");
